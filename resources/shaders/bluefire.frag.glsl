...
float func(vec2 p){
    ...
    // The noise value.
    float n = tetraNoise(vec3(p.x*4., p.y*4., 0) - vec3(0, .125, .025)*iTime);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // Screen coordinates.
	vec2 u = (fragCoord.xy - iResolution.xy*.25)/iResolution.y;
    ...
    vec3 col = pow(min(vec3(1.35, 1, 1)*(f*.7 + ssd*.35), 1.), vec3(1, 2., 5)*2.) + .01;
