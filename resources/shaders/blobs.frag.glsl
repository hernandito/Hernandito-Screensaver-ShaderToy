...
float cos2(float x){return cos(x-sin(x)/1.);}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    diff+=vec3(.1,.1,0)*smoothstep(-.13,0.7,-rp.y+0.)*max(0.,-n.y)*.07;
    ...
    // fresnel
    diff+=pow(clamp(1.-dot(-rd,n),0.,.0),0.)*.005;

    // background and floor fade
    vec3 backg=vec3(0,0,0);
    ot.x+=.6+iTime/19.;
    ot.y+=cos(floor(ot.x*2.)*3.)*.1+.2;
    ot.x=mod(ot.x,.05)-.025;
 
    diff=mix(diff,backg,smoothstep(.09,9.,t));

    fragColor.rgb=mix(vec3(.0,0,0),vec3(1),ao)*shad*diff*1.0;
    fragColor.rgb+=emit;
